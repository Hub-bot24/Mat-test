...
function save(){
  ...
  d._tri=tri;
  localStorage.setItem('page1', JSON.stringify(d));
}
function load(){
  try{ const d=JSON.parse(localStorage.getItem('page1')||'{}');
    Object.entries(d).forEach(([k,v])=>{ if(k==='_tri')return; const el=document.getElementById(k); if(el) el.value=v; });
...
function computeTotal(){
  ...
  const t=Math.round(n('bitumen')+n('kerosene')+n('additive'));
  const o=document.getElementById('totalLitres');
  if(o) o.value = t ? String(t) : '';
  save();
}
...
function initTri(){
  document.querySelectorAll('.tri').forEach(el=>{
    el.addEventListener('click', ()=>{
      if(el.classList.contains('state-1')){ el.classList.remove('state-1'); el.classList.add('state-2'); }
      else if(el.classList.contains('state-2')){ el.classList.remove('state-2'); }
      else { el.classList.add('state-1'); }
      save();
    });
  });
}
