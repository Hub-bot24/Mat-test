...
// No service worker. Self-heal code in HTML will unregister any old SW once.
// Storage
const SKEY = 'mtp_data_v325';
...
function initPage6(){
  ...
  ['m1','m2','a','dl'].forEach(id=>{ const el=document.getElementById(id); el.addEventListener('change', calc); el.addEventListener('input', ()=>{ const data=getStore(); data[id]=el.value; setStore(data); }); });
  initPDF('pdf6');
  calc();
}
